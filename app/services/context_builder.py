...
class ContextBuilder:
    """
    Trusted service that builds bounded execution context for an agent invocation.
    Retrieves state from MySQL and constructs the multi-layered prompt.
    """
    ...
    def build_context(
        ...
        # 2. Extract Project Metadata & Context
        project_info = None
        if project:
            project_info = {
                "id": project.id,
        ...
        # 4. Resolve Token Budget & Allowed Tools
        ctx_policy = agent_version.context_policy or {}
        token_budget = ctx_policy.get("max_tokens", 4096)
